...
async def stt_streaming_websocket(websocket: WebSocket):
    ...
    try:
        ...
        logger.info("STT streaming WebSocket disconnected")
    except Exception as e:
        logger.error(f"STT streaming WebSocket error: {e}")
    finally:
        try:
            await websocket.close()
            logger.info("STT streaming WebSocket connection closed")
        except:
            pass
