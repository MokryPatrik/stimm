...
class LiveKitService:
    ...
    async def start_sip_monitoring(self):
        ...
        logger.warning("⚠️ LiveKitService.start_sip_monitoring() is deprecated. "
                      "SIP monitoring is now handled by SIPBridgeIntegration.")
        return
    ...
    async def _spawn_agent_for_sip_room(self, room_name: str) -> bool:
        ...
        logger.warning(f"⚠️ LiveKitService._spawn_agent_for_sip_room({room_name}) is deprecated. "
                      "Use SIPBridgeIntegration instead.")
        return False
