...
from database import get_db, User, RagConfig
...
class RagConfigService:
        ...
        for prop in expected_properties:
            if prop not in config:
                raise AgentValidationError(
                    f"Missing required property '{prop}' for RAG provider '{provider_name}'"
                )
    ...
    def _validate_rag_config_create(self, rag_config_data: RagConfigCreate) -> None:
        ...
        self._validate_provider_config(
            rag_config_data.provider_config.provider,
            rag_config_data.provider_config.config
        )
    ...
    def create_rag_config(self, rag_config_data: RagConfigCreate, user_id: Optional[UUID] = None) -> RagConfigResponse:
        ...
        try:
            ...
            existing_config = session.query(RagConfig).filter(
                and_(
    ...
    def list_rag_configs(
        ...
        try:
            ...
            if active_only:
                query = query.filter(RagConfig.is_active == True)
        ...
        try:
            ...
            if rag_config_data.provider_config is not None:
                ...
                if rag_config_data.provider_config.provider != rag_config.provider:
                    ...
                    update_fields['provider_type'] = self._determine_provider_type(
                        rag_config_data.provider_config.provider
                    )
